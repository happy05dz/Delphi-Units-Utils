...
const
    ...
    AnsiremovecursorPattern = '\e\[(K|s|u|2J|2K|\p{Nd}+(A|B|C|D|E|F|G|J|K|S|T)|\p{Nd}+;\p{Nd}+(H|f))';
    MobilDZPattern        = '^(00213|\+213|0)(5|6|7)[0-9]{8}';
...
begin
try
	ResultString := TRegEx.Replace(SubjectString, AnsiremovecursorPattern, '', [roMultiLine]);
  Result := ResultString;
  except
...
begin
try
	if TRegEx.IsMatch(Phonenumber, MobilDZPattern) then
  Result := true
  else
   Result := false;
  except
